...
namespace BigEgg.Presentation
{
    public class TaskbarItemOverlay
    {
        public static readonly DependencyProperty ContentProperty = DependencyProperty.RegisterAttached("Content", typeof(object), typeof(TaskbarItemOverlay), new PropertyMetadata(OnPropertyChanged));
        ...
        private static void OnPropertyChanged(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (dependencyObject is TaskbarItemInfo)
            {
                taskbarItemInfo = dependencyObject as TaskbarItemInfo;
            }
            else
            {
                return; 
            ...
            RenderTargetBitmap bmp = new RenderTargetBitmap(ICON_WIDTH, ICON_HEIGHT, 96, 96, PixelFormats.Default);
